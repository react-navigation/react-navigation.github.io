...
Then we can add a [`header` navigation option](/content/docs/navigation-options.html) that allows us to add a custom right button:
...
The navigation options can be defined with a [navigation prop](/content/docs/navigation-prop.html). Let's render a different button based on the route params, and set up the button to call `navigation.setParams` when pressed.
...
To see the rest of the header options, see the [navigation options document](/content/docs/navigation-options.html).
