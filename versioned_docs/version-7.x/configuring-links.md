...
If you wanted to resolve `/user/jane/settings` to result in the params `{ id: 'user-jane' section: 'settings' }`, you could make `Profile`'s config to look like this:
...
```js
// For the URL /user/jane/settings
const state = {
  routes: [
    {
      name: 'Profile',
...
If the URL contains a `section` param, this will result in the following with the same config:
...
A path like `/library` or `/settings/notification` will resolve to the following state object:
