...
Previously the types for the root navigator were specified using `declare global` and `RootParamList`. Now, they can be specified with module augmentation of `@react-navigation/native` and use the navigator's type instead of a param list:
...
If you use the dynamic API, you'll need to pass the type of the nested navigator to `NavigatorScreenParams` for typed hooks to infer correct types based on screen:
