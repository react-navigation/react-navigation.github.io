...
We've continued working on React Navigation 8.0 since the [initial announcement](/blog/2025/12/19/react-navigation-8.0-alpha) and [March progress report](/blog/2026/03/10/react-navigation-8.0-march-progress) and released many refinements, improvements and new features.
...
Screens now have experimental support for data loaders. You can use loaders to prefetch data for a screen before it renders. For nested navigation, they run in parallel for all intermediate screens, avoiding waterfalls.

Loaders can be defined with the [`UNSTABLE_loader`](/docs/8.x/screen#loader) property:
...
[`@react-navigation/native-stack`](/docs/8.x/native-stack-navigator) will use the next major version of [`react-native-screens`](https://github.com/software-mansion/react-native-screens) library which is currently in development. We plan to release a beta version once we have the new `react-native-screens` integration ready.
...
```sh npm2yarn
npm install @react-navigation/native@next @react-navigation/native-stack@next @react-navigation/bottom-tabs@next
```
